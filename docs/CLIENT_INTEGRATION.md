...
A client binds explicitly to the service package and action:
...
The client manifest currently needs:
...
## Current signing requirement

The permission is currently declared by Android LLM Service with `protectionLevel="signature"`. Android therefore grants it only to apps signed with the same signing certificate as the service.

This is suitable for coordinated builds and prevents arbitrary installed apps from consuming the model/runtime. It is not sufficient for independent distribution channels where client and service packages receive different signing keys.

Issue #10 tracks a public-client authorization model. Until that is implemented, independently signed builds should be considered unsupported rather than weakening the permission locally.
...
2. Service exists but binding is not authorized.
...
- Android LLM Service is installed but this build is not authorized to use it.
